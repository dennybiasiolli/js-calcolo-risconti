    ...
    .controller('myCtrl', ['$scope', '$window', function ($scope, $window) {
        $scope.calcolaRisconti = function () {
            ...
            if ($scope.DataInizio && $scope.DataFine) {
                ...
                $scope.diffGiorni = Math.ceil(timeDiff / (1000 * 3600 * 24)) + ($scope.IncludiDataInizio ? 1 : 0);
                if ($scope.diffGiorni > 0) {
                    ...
                    for (var annoAttuale = annoInizio; annoAttuale <= annoFine; annoAttuale++) {
                        ...
                        var addGiornoCalc = (annoAttuale == annoInizio && !$scope.IncludiDataInizio ? 0 : 1);
        ...
        $scope.DataFine.setDate($scope.DataFine.getDate() - 1);
        $scope.IncludiDataInizio = true;
